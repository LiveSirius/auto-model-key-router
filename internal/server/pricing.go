...
//
// **为什么挂在 /ui/ 而不是新开一条 /api/ 路由**：管理面的 47 条是参照实现留下的
// 已发布接口，运维面的 7 条与 app 面的 13 条也各有对外承诺。新增一条 /api 路由就得
// 手写一条对它负责的用例，而那些用例只能断言「新写的代码是新写的」——给不出兼容性
// 证据。/ui/ 挂载在所有已发布清单之外，且已经有一套"可用/启用"的判断逻辑，把只读
// 文档放在这里不需要碰任何清单。
//
