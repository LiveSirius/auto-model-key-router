...
//
// **为什么挂在 /ui/ 而不是新开一条 /api/ 路由**：管理面的 47 条与运维面的 7 条都是
// 有对外承诺的已发布接口，它们的响应形状不能随意增减（当年的逐字节语料由已退役的
// Python 脚本生成，已随参照实现一起删除）。自更新是 Go 版**新增**的能力，Python 侧
// 没有对应实现，因此新写的用例只能断言「新写的代码是新写的」，给不出兼容性证据。
// 这与 pricing.go 选择的出路相同：挂在 /ui/ 这个不在已发布清单里的前缀下。
//
