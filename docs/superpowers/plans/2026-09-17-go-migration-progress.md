...
> ## ✅ 迁移已完成（2026-09-18）
>
> **Python 已整体退役，仓库只剩 Go 实现与共享资产。** 终态核验：
>
> | 项 | 结果 |
> | --- | --- |
> | `git ls-files '*.py'` | **0**（工作区残留也是 0） |
> | Go 代码 | 219 文件 / 68,416 行，29 个包 |
> | 前端资产 | `webui/` 21 文件（18 个静态资产 + 3 个 node 探针） |
> | `go build` / `go vet` / `gofmt -l .` | 全部干净 |
> | `go test ./...` | **29 个包全过** |
> | 语料 | 23 份**冻结**差分语料留在 `internal/*/testdata/` 与 `cmd/amkr/testdata/`，Go 测试逐条回放 |
> | CI | 两个作业：`webui`（node 检查 + 3 个探针）与 `go`（gofmt / vet / test） |
> | 仅服务 Python 的脚本 | 已全部移除（23 个生成器、release.py、预览脚本、release.yml、publish-pypi.yml） |
>
> 关键提交：`dced5ae`（退役 23 个生成器）、`1393244`（退役 Python 实现）、`fa67ab7`（README 改 Go）。
>
> **与原计划的偏差**：目标写的是「按模块**依次**删除」，但实测表明剩余 33 个 Python 模块是一个
> 互相依赖的连通块（`dashboard`/`logs_tui` 是仅有的两个真叶子，已先删），无法逐个删除，
> 只能整体退役。结论与依据见下文「阶段 3」的更正章节；已获用户确认后执行。
>
> **一处未复现的观察**：终态核验期间有一次 `go test ./...` 报 28 ok / 3 FAIL，紧接着的重跑与
> 另一次独立重跑都是 29 ok / 0 FAIL，未能复现。可疑方向是 `internal/eventbus`（有两个真实
> 等待 10 秒的用例）与带时序断言的 `internal/server` / `internal/tui`。若 CI 上偶发失败，
> 应优先查这几处，而不是怀疑语料。
>
> 以下为迁移过程中的原始记录，保留供追溯。
